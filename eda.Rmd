...
# xx <- platform_data_classified %>%   # For testing
  ...
  # filter(customer_email == "1030Lashaunfleming@gmail.com") %>%  # For testing mixed carts
  ...
  mutate(transaction_within_past_60_days = if_else(condition = max_transaction_date > (Sys.Date() - 60),
                                                   true = "Yes",
                                                   false = "No"),
         customer_lost = if_else(condition = (!is.na(max_cancel_date) & customer_transaction_number == total_transactions & order_date < max_cancel_date) |
                                   (!is.na(max_cancel_date) & customer_transaction_number == total_transactions & transaction_within_past_60_days == "No")
                                 ,
                                 true = "Lost",
                                 false = "Retained")) %>% 
  rowwise() %>% 
...
yy <- tt %>% filter(customer_email %in% c(head(yy)))
