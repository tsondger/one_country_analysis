...
platform_data_classified <- platform_data %>% 
  mutate(alt_entry = if_else(condition = customer_email_lower %in% c(alt_entry_customers$email),  # Comment when done
                             true = "Yes",
                             false = "No"),
         # Going to prioritize alt entry over member and one-time, add the bonus entries when that comes in
...
# KEEP BELOW IN HERE TO JUSTIFY LOWERE CASING THE EMAISL
...
# 1. After classifications - ID first purchase and classify cohort DONE
# 2. ID when the customer is lost DONE BUT still evaluating
# 3. Once we get to the point of aggregation also aggregate the product sales
...
platform_data_cohorts <- platform_data_classified %>% 
  ...
  # Follows the text definition - but I don't think that's the way we should define it
  group_by(customer_email) %>% 
  mutate(total_transactions = max(customer_transaction_number),
         customer_lost = if_else(condition = !is.na(canceled_date) & customer_transaction_number == total_transactions,
                                 true = 1,
                                 false = 0)) %>% 
  ungroup()

# Figure out how to ID if any customer is lost to churn
