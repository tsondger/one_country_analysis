...
platform_data <- bind_rows(big_comm, bold_v1, bold_v2, give_orders, mini_bc, shopify) %>% 
...
platform_data_classified <- platform_data %>% 
  mutate(alt_entry = if_else(condition = customer_email_lower %in% c(alt_entry_customers$email),  # Comment when done
         ...
         grp = case_when(
           product_details %in% c(products_classified$product_details[products_classified$grp == "Member"]) | 
             file_source == "Boldv2.csv" ~ "Member",
...
# xx <- platform_data_classified %>%   # For testing
  # filter(customer_email %in% c("Kylepyeoman@gmail.com", "trendaadkins@yahoo.com", "hopeangeltaylor@icloud.com",
                               # "0.betwixbeginningbeyond.1@gmail.com", "001sharpshot@gmail.com", "005075118@coyote.csusb.edu", "007boyce.b.hamilton@gmail.com"
                               # )) %>% ## For testing - comment out
  arrange(customer_email, order_date) %>% 
  group_by(customer_email) %>% 
  mutate(customer_transaction_number_classify = if_else(condition = grp %in% c("Member", "One-time"),  # Testing
                                                        true = row_number(),
                                                        false = as.integer(0)),
  ...
  # mutate(customer_transaction_number = row_number()) #%>%  # Original
  ungroup() %>% 
  mutate(cohort = if_else(condition = customer_transaction_number_classify == first_customer_order,  # Testing
                          true = paste0(grp, " ", month(order_date, label = T), "/", year(order_date)),
                          false = ""),
         cohort = na_if(cohort, ""),
         first_order_date = if_else(condition = customer_transaction_number_classify == first_customer_order,
                                    true = order_date,
                                    false = ymd("2099-1-1")),
  ...
  # mutate(cohort = if_else(condition = customer_transaction_number == 1,  # Original
  #                         true = paste0(grp, " ", month(order_date, label = T), "/", year(order_date)),
  #                         false = ""),
  #        cohort = na_if(cohort, ""),
  ...
  #                                   false = ymd("2099-1-1")),
  #        first_order_date = na_if(first_order_date, ymd("2099-1-1")),
  #        cohort_grp = if_else(condition = customer_transaction_number == 1,
  #                             true = grp,
  #                             false = ""),
  #        cohort_grp = na_if(cohort_grp, ""),
  #        cohort_sub_grp = if_else(condition = customer_transaction_number == 1,
  #                                 true = sub_grp,
  #                                 false = ""),
  #        cohort_sub_grp = na_if(cohort_sub_grp, "")) %>% 
...
# xx <- tt %>% filter(months_since_first_purchase < 0)
